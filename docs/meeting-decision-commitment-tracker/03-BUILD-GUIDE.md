...
- Maker access to [https://make.powerapps.com](https://make.powerapps.com) and [https://make.powerautomate.com](https://make.powerautomate.com).
...
1. Reorder the groups so **Meeting**, **Decision**, **Commitment** come first: select a group in the Pages tree and drag it, or use its **⋯** menu → **Move up** / **Move down**.
2. Make **Team Member** read as reference data rather than a primary work area: select the Team Member group → the rename (pencil) icon → change the group's display label to **Reference Data**. (This only relabels the nav group — the underlying table is unchanged.)
...
1. On `scrCommitmentDetail`: **Insert** → **Edit form**, `X = 20`, `Y = 90`, `Width = 900`, `Height = 500`, **Data source** = `Commitments`, rename `frmCommitment`. Formula bar → **Item**: `varSelectedCommitment` (same reason as `frmMeeting` in B.3 step 1 — `EditForm()` sets mode, not which record). **Insert** → **Back arrow** icon at `X = 20, Y = 20` → **OnSelect**: `Navigate(scrCommitments, ScreenTransition.Fade)`.
  **Field cleanup (same as B.3):** select `frmCommitment` → right panel **Fields** → **Edit fields** → remove the auto-added system **Status** (`statecode`) and **Status Reason** (`statuscode`) fields — distinct from the business **Commitment Status** column you designed; leaving them in throws **"Column 'statecode' is generated by the server and can't be specified"** on Save. Also remove any other system/plumbing fields Copilot added (e.g. Record Created On, Import Sequence Number, Time Zone Rule Version Number) that aren't in 02-DATA-MODEL.md's Commitment column list.
   **Placeholder-data gotcha:** if the Meeting, Commitment Owner, or Commitment Status field cards show fake data at runtime (generic "Item 1"/"Item 2"... entries, or a raw number like `100000000` instead of a status label) instead of your real records, their cards are stale — most likely from renaming the Commitment Owner column earlier. Fix: **Fields** → **Edit fields** → remove those fields → re-add them from the same panel, then **Save** → **Publish**. This forces Studio to regenerate the cards against the current schema.
...
5. **Insert** → **Vertical gallery** (layout: "Title, subtitle, body" so you get three text placeholders to repurpose below). Position: `X = 20`, `Y = 190`, `Width = 1326`, `Height = 530`. Rename `galCommitments`. Formula bar → **Items**:
   ...
   **Naming note:** the lookup is deliberately named **Commitment Owner**, not plain "Owner" — every Dataverse table also has a system-managed, polymorphic **Owner** column (`ownerid`), and referencing bare `Owner` here would resolve to that system column instead, throwing **"The '.' operator cannot be used on Polymorphic values"** the moment you try `.` on it. If you already created the lookup as plain "Owner," rename its display name to **Commitment Owner** (table → column settings) before using these formulas.
   Set the three auto-generated placeholder labels via the canvas, not the **Fields** panel (its per-card boxes only accept picking a column from the chevron dropdown, not typed formulas):
  - Click `galCommitments` once, then click its **first row** to drill into the template.
  - Click the **Title** label (e.g. `Title8`) → top formula bar → property dropdown **Text** → `ThisItem.'Commitment Title'` → Enter.
  - Click the **Subtitle** label (e.g. `Subtitle8`; this becomes the due-date label referenced in step 6 below) → **Text** → `"Due " & Text(ThisItem.'Due Date', "yyyy-mm-dd")` → Enter.
  - Click the **Body** label (e.g. `Body2`) → **Text** → `"Owner: " & If(IsBlank(ThisItem.'Commitment Owner'), "(none)", ThisItem.'Commitment Owner'.'Team Member Name')` → Enter. `Commitment Owner` is optional (blank = ownerless per 02-DATA-MODEL.md), so guard with `IsBlank()` before dereferencing `.'Team Member Name'`, otherwise a blank Commitment Owner throws on an ownerless commitment.
...
3. **Filter rows** is a plain OData text field in this connector version — there's no graphical condition builder here, so it must use each column's actual **logical name** (lowercase, no spaces), not the display name:
  `new_commitmentstatus ne 100000003 and new_commitmentstatus ne 100000004 and new_duedate eq '@{formatDateTime(utcNow(),'yyyy-MM-dd')}'`
   **Why not the display names ("Commitment Status", "Due Date"):** OData property names can't contain spaces — typing the display name throws a syntax error at the first space. The schema names follow this environment's `new_` + column-name-without-spaces pattern (e.g. `new_MeetingName`), and Dataverse's Web API always lowercases logical names regardless of the mixed-case schema name shown in the maker portal, hence `new_commitmentstatus` / `new_duedate`.
   **Why numbers, not** `'Done'`**/**`'Cancelled'`**:** `Commitment Status` is a Choice column — Dataverse stores and filters Choice values as their underlying numeric **Value**, not the label text. Per this table's Commitment Status choices, `Done = 100000003` and `Cancelled = 100000004`.
   **Why** `eq` **today, not** `lt` **now:** this sends one reminder per commitment, only on its due date — `Due Date` is a **Date only** column, so `eq` against today's date (`yyyy-MM-dd`) matches it exactly once and never again on later days.
...
3. **Table name**: Team Members. **Row ID**: pick **Commitment Owner (Value)** from dynamic content on the Apply to each item — Dataverse lookups surface as a pair of tokens, `(Type)` (which table it points to) and `(Value)` (the actual GUID); `Get a row by ID` needs the `(Value)` one.
  **Nested-loop gotcha:** if the dynamic content picker's "Commitment Owner" entry actually resolves to **List rows'** raw, unfiltered output rather than the current item of *this* Apply to each, Power Automate silently auto-wraps the action in a second, unwanted "Apply to each" (its own `Select an output from previous steps` will read `value`) — looping every commitment's owner again for every outer iteration instead of just the current one. If you see an inner "Apply to each" appear that you didn't add, delete it (and the action inside it), re-add **Get a row by ID** as a plain step directly in the outer loop, and set Row ID via the **Expression** tab instead of the picker:
   `items('Apply_to_each')?['_new_owner_value']`
   (schema name `new_Owner` per the Commitment table's column list — swap in your own lookup's schema name and loop name if they differ).
4. Still inside the loop, **+ Add an action** → **Dataverse** → **Get a row by ID** again (rename it, e.g. "Get meeting"). **Table name**: Meetings. **Row ID**: pick **Meeting (Value)** from dynamic content, same reason as step 3 — `(Value)` is the Meeting row's GUID, `(Type)` is just metadata. If the same nested-loop gotcha appears here, fix it the same way, using the Expression tab:
  `items('Apply_to_each')?['_new_meeting_value']`
   Every Commitment has a required Meeting lookup (02-DATA-MODEL.md), so this step never fails on a missing meeting — this is what lets the email name the meeting a commitment came from instead of reading like a bare to-do.



### C.5 Send the email

1. Still inside the loop, right before Send an email, **+ Add an action** → **Compose** (rename it "Log reminder attempt"). Click into its **Inputs** field → open the **Expression** tab (next to Dynamic content, not the plain text box) → paste the whole formula below → **OK**:
   `concat('Reminder attempt — Owner: ', body('Get_owner')?['new_Email'], ' | Meeting: ', body('Get_meeting')?['new_MeetingName'], ' | Commitment: ', items('Apply_to_each')?['new_CommitmentTitle'], ' | Due: ', items('Apply_to_each')?['new_DueDate'])`
   **Use `concat()`, not `&`:** Power Automate's cloud-flow expression language has no `&` concatenation operator — that's Power Fx (canvas apps, Track B) only. Typing `"..." & body(...) & "..."` straight into the plain Inputs box gets stored as inert literal text (never evaluated), which is why a Compose built that way echoes the raw unresolved formula back as its output instead of a real value. Entering `concat(...)` via the **Expression** tab is what makes Flow evaluate it as a live expression.
   **Action names need underscores, not spaces:** an action's display name can show spaces ("Get owner"), but the template's internal identifier — what `body()`/`items()`/`outputs()` must reference — always has spaces replaced with underscores (`Get_owner`, `Apply_to_each`). Referencing the display name as typed throws `InvalidTemplate: ... are not defined in the template` on save. Safer than guessing the exact identifier: type just `body(` in the Expression editor and use its autocomplete list to pick the action, rather than hand-typing its name.
   Logging *before* the send means this line still appears in run history even if the email step itself fails (bad address, throttling, connector error) — so a developer debugging a partial run can immediately tell which reminder was attempted right before things broke, without that information being lost along with the failed action's own inputs.
2. **+ Add an action** → **Office 365 Outlook** → **Send an email (V2)**.
3. **To**: the **Email** field from the **Get owner** step (dynamic content) — the Team Member's fictional/dummy address.
4. **Subject**: `Overdue commitment from "` + dynamic content **Meeting Name** from the **Get meeting** step + `":`  + dynamic content **Commitment Title** from the Apply to each item.
5. **Body**: **Meeting Name** and **Meeting Date** (from **Get meeting**) first, so the Owner immediately places which meeting this is about, then **Commitment Title** and **Due Date** (from the Apply to each item). Keep it to those four fields, plain text — no status dropdowns, priority flags, or action buttons; per the scope rule above, this stays a nudge, not a task-management notification.
6. Still inside the loop, right after Send an email (V2), **+ Add an action** → **Compose** (rename it "Log reminder sent"). Same as step 1 — click into **Inputs** → **Expression** tab → paste → **OK**:
